        ...
        if (withCap.length > 0) {
          ...
          if (seniorRelaxed.length > 0) {
            ...
            if (fullRelaxed.length > 0) {
              ...
              // 全員上限超え：やむを得ず上限を緩める
              candidates = base;
              warnings.push(`${dateStr} 当直: 週末シフト上限超過のため上限を緩めて割り当てます`);
            }
          }
        }
      }
